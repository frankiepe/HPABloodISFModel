...
import json
from methods.models import HPAModelFEInterBothCBGAlbBloodISF
from methods import day_len

# Define parameter config file path
config_file = 'configs/HPAModelFEInterBothCBGAlbBloodISF/test_parameters.json'
...
# Initialise model
dde_model = HPAModelFEInterBothCBGAlbBloodISF(parameters=parameters, times=times, num_days=num_days, days_to_keep=days_to_keep, step=step)
...
ax2.plot(plot_times, result.T[9], label='Free Cortisol', color='blue', alpha=0.5)
ax2.plot(plot_times, result.T[10], label='Free Cortisone', color='red', alpha=0.5)
ax3.plot(plot_times, result.T[0], label='ACTH', color='orange')
...
ax5 = ax2.twinx()
ax5.plot(plot_times, crh_drive, color = 'grey', alpha = 0.4)
ax5.set_ylabel('CRH drive', color = 'grey')

ax6 = ax3.twinx()
ax6.plot(plot_times, crh_drive, color = 'grey', alpha = 0.4)
ax6.set_ylabel('CRH drive', color = 'grey')
...
plt.savefig(f'figures/model_output/HPAModelFEInterBothCBGAlbBloodISF/test_sim.png')
