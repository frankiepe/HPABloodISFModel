...
function HPAModelFEInterCBGAlbBloodISF(du, u, h, p, t)
    gamma_a, gamma_f_b, gamma_f_i, gamma_e_b, gamma_e_i, K_a, K_f, K_mf, K_me, k_Fon, k_Foff, k_Eon, k_Eoff, k_BI_F, k_BI_E, m_a, m_f, V_f, V_e, 
    ...
    du[2] = -(gamma_f_b + k_Fon) * u[2] + alpha*((u[1]^m_f)/((K_a^m_f) + (u[1]^m_f))) + k_Foff * u[4] + (V_e * u[3])/(K_me + u[3]) - (V_f + u[2])/(K_mf + u[2])
            - (k_BI_F/V_B)*(u[2]-u[6])
    du[3] = -(gamma_e_b + k_Eon) * u[3] + k_Eoff * u[5] - (V_e*u[3])/(K_me+u[3]) + (V_f+u[2])/(K_mf+u[2]) - (k_BI_E/V_B)*(u[3]-u[7])
    du[4] = k_Fon * u[2] - k_Foff * u[4]
    du[5] = k_Eon * u[3] - k_Eoff * u[5]
    du[6] = (k_BI_F/V_I)*(u[2]-u[6]) - gamma_f_i*u[6]
    du[7] = (k_BI_E/V_I)*(u[3]-u[7]) - gamma_e_i*u[7]
end

function HPAModelFEInterBothCBGAlbBloodISF(du, u, h, p, t)
    gamma_a, gamma_f_b, gamma_f_i, gamma_e_b, gamma_e_i, K_a, K_f, K_mfB, K_meB, K_mfI, K_meI, k_Fon, k_Foff, k_Eon, k_Eoff, k_BI_F, k_BI_E, m_a, m_f, V_f_b,
    ...
    du[2] = -(gamma_f_b + k_Fon) * u[2] + alpha*((u[1]^m_f)/((K_a^m_f) + (u[1]^m_f))) + k_Foff * u[4] + (V_e_b * u[3])/(K_meB + u[3]) - (V_f_b + u[2])/(K_mfB + u[2])
            - (k_BI_F/V_B)*(u[2]-u[6])
    du[3] = -(gamma_e_b + k_Eon) * u[3] + k_Eoff * u[5] - (V_e_b*u[3])/(K_meB+u[3]) + (V_f_b+u[2])/(K_mfB+u[2]) - (k_BI_E/V_B)*(u[3]-u[7])
    du[4] = k_Fon * u[2] - k_Foff * u[4]
    du[5] = k_Eon * u[3] - k_Eoff * u[5]
    du[6] = (k_BI_F/V_I)*(u[2]-u[6]) - gamma_f_i*u[6] + (V_e_i*u[7])/(K_meI+u[7]) - (V_f_i+u[6])/(K_mfI+u[6])
    du[7] = (k_BI_E/V_I)*(u[3]-u[7]) - gamma_e_i*u[7] - (V_e_i*u[7])/(K_meI+u[7]) + (V_f_i+u[6])/(K_mfI+u[6])
end
